...
| `fix/dropdown-button-padding` | The labelled button in a `ShrinkableDropdownButton` had 21px between its label and the caret beside it, against 11px on the caret's own far side — a pair weighted towards the gap inside the group rather than the one around it. 0.5em on that side now. Only the labelled state: shrunk to its icon the button is square already. | Fixes upstream behaviour, though the component is this fork's to the extent that it added the group. |
| `fix/toggle-needs-two-clicks` | Reloading while Wide was on took **two clicks** to turn it off. The preference was read at render only, as a default for a destructuring of `this.state` — so `this.state.wide` stayed `undefined` until something wrote to it, and `toggleWide` derives the next value from `this.state`: `!undefined` is `true`, so the first click set wide to what it already was. Reproduced and fixed against the live page. The theme toggle had the same shape and was saved from it by accident: `init()` happens to re-seed `state.theme` from storage on connect, so it was only reachable by clicking before the hub connected. Both are seeded in the constructor now, and the render defaults — the second source of truth that let the two disagree — are gone. | Fixes upstream behaviour; `wide` is this fork's, the theme's latent half is upstream's. |
| `feature/clear-searches` | A Clear on the searches list that removes the finished, unwatched ones. **Watched searches are kept because nothing cascades**: `SearchService.DeleteAsync` removes the search and leaves the schedule, the memory of what it has already reported and its notifications pointed at something that is gone — which is the same reason `PruneAsync` already excludes them, said in its own comment. Running searches are kept too, since the row's own control refuses to delete one and offers Stop instead; a button that does in bulk what the single action will not is a surprise rather than a convenience. The confirmation names what is kept and tells those two reasons apart. | Ours. |
| `fix/subtle-contrast` | `--slskd-color-subtle` measured 4.12 / 3.77 / 3.31 against the three dark backgrounds it is used on — under the 4.5 body text wants on all of them. It is not only decoration: the patched stylesheet hands it to every toggle's label, so Table View and Hide Results with No Free Slots were reading at 4.12 as well as the card and item meta. Raised to `#8b949e`, which measures 6.16 / 5.63 / 4.95 and still sits at 1.99 against the body colour — visibly dimmer, which is the point of a subtle colour. It is GitHub's own muted foreground, where the rest of this palette comes from. | The gap is upstream's theme patch. |
| `fix/dark-item-header` | The rest of the dark theme, found by sweeping every page rather than waiting to be told. The theme is a patch over Semantic's stylesheet replacing hard-coded colours with variables, and what it missed is invisible until somebody opens that page in that theme: the **user card's header** (`.ui.items > .item > .content > .header`, kept at `#000000d9` while its own description followed the theme — contrast 1.11), the **column picker's arrows and border** (basic buttons, `rgba(0,0,0,.6) !important`, portalled out of the bar the earlier rule was scoped to), the **`positive` and `active` row states** (`#fcfff5` and `#e0e0e0` — a near-white band across a dark table, which is what a downloaded track looked like; `warning` was already answered and these follow its pattern), and the **conversation and room lists** at 1.72. `tests/web-perf/dark-contrast-sweep.mjs` keeps the measurement. | The gaps are upstream's theme patch; finding them is ours. |
| `fix/dark-table-headers` | In dark mode every sortable column header was black on dark — measured contrast **1.21**, which is not a dim header but an invisible one — and the Columns button 1.11. Semantic hard-codes those colours rather than taking them from a variable, so the theme patch never reached them, and this fork is what made the tables sortable. The button rule is scoped to the summary bar rather than to every basic button, since Semantic colours a *coloured* basic button with a rule of the same shape. The browse card's username links to the user page with it: the user card already links to their files, and this is the other direction. | The dark-theme gaps are upstream's, exposed by a table this fork made sortable. |
| `feature/column-ordering` | The columns can be reordered, with arrows in the picker, on both tables. `parseColumns` had been re-sorting a stored list into this file's order on the grounds that a column list is a set — true until the order became something an operator could choose, at which point discarding it discards the choice. Switching a column *on* still puts it where this file would have it, before the first shown column that follows it, since appending would move a column for anyone who has never reordered anything; and it reads the shown order rather than rebuilding from the canonical one, so a column that has been moved stays moved. Arrows rather than dragging: a drag needs a pointer and this list is read on a phone too. They point up and down rather than left and right — what moves is the row being pointed at, in the vertical list it sits in; the column moving left in the table is the consequence, not the gesture. The picker is shared now instead of a copy in each table. | Ours — the tables are this fork's. |
| `fix/table-sidescroll-mobile` | The table scrolls sideways on a phone rather than being cut off. Unstacking it and relaxing the widths was not enough on its own: at 375px the columns still want 613px between them, and everything past the viewport was clipped by `.pusher.app-content`, which is `overflow-x: hidden` — so the last columns were not merely cramped, they were unreachable. Applied at every width rather than below a breakpoint: whether the table fits depends on which columns are shown, not on the window, and at exactly 768px — one pixel outside the phone rule this started as — 35px was still clipped with nothing to scroll. The load-bearing half is `overflow-y: hidden` rather than `visible`: a box with `visible` on one axis computes it to `auto` when the other is not visible, and `scrollParentOf` matches `auto`/`scroll`/`overlay`, so the wrapper would be taken for the page's scroller and the virtualised list would draw the first screenful forever. | Ours. |
| `fix/table-view-mobile` | Below 768px Semantic stacks a table into blocks — every cell `display: block; width: 100%` — which is ruinous for a virtualised one whose rows are positioned by arithmetic on a fixed height: measured at 600px, each header cell was 538px wide inside a 540px table while File and Path were 19px. `unstackable` is Semantic's own opt-out. The fixed column widths are relaxed there too, rather than scrolled sideways: a wrapper with `overflow-x: auto` computes `overflow-y` to `auto`, which is exactly what `scrollParentOf` matches on, so the virtualiser would measure itself against a box that never scrolls. **And the filename was the only column that could give** — every other one carries a fixed width, so the shortfall came out of the two that did not ask for one; floors under File and Path share it out. Four columns narrowed besides: Ext 70→42, User 180→117, Search 190→105, Size 100→70, actions 96→82. | Ours — the table view is this fork's; `unstackable` is upstream's own answer. |
| `fix/users-wide-and-browse-link` | Two things the Users page was missing. **Wide did not reach it**: the toggle lifts the cap on `.view`, and `/users` and `/browse` are the two routes not wrapped in one, so the page stayed at 1200px whatever the setting said. And the browse link added to search results was not on the user card, which is the page most likely to raise the question — it answers "who is this", and their shares are the obvious next thing to ask for. The link is a shared `BrowseLink` now rather than a copy in each card. | Ours. |
| `feature/browse-by-url` | `/browse/:username?`, so a user's shares can be linked to — and a search result's card carries a link to its peer's, which is the whole reason to read a card rather than a flat list and previously meant copying the name to another page. Everything that starts a browse goes through the route, so `submit` only changes the address and the update reacts to it. The IndexedDB restore stays, since a browse fetches an entire share, but stands aside for an address that names someone. **And a bare `/users` is an empty form again**: it reopened whoever was looked up last, so the menu item took you to a user rather than to the page — putting the name in the route made that visible, but the remembering was the part that was wrong. | The routes would stand alone upstream. |
